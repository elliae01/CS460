...
def DisplayTracer(display_Surface, tracer, heading, targetVisible, targetLocation, targetSymbol, locSymbol):
	...
	if (targetVisible == 1):
		...
		print("Place Symbol",targetLocation.getX(),targetLocation.getY(), pastElement.getX(),  pastElement.getY())
		placeSymbol(display_Surface, targetSymbol, targetLocation.getX() + 325, targetLocation.getY() - 280)
...
def DBGUI_main(startDateTime, endDateTime):
	...
	# Date time of Final Demo - Aaron's Shot
	StartDate = pd.to_datetime('2018-04-21 17:51:30.000')
	EndDate = pd.to_datetime('2018-04-21 18:58:03.500')

	# Date time of Final Demo - Kyle's ZigZag
	StartDate = pd.to_datetime('2018-04-21 18:04:30.000')
	EndDate = pd.to_datetime('2018-04-21 18:05:03.500')

	T = Targalytics(DatabaseInfo, StartDate, EndDate)
	T.ExportToCSV('Final Demo')
