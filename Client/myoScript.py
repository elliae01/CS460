...
from common import *
from accessor import accessorMethods
...
functions could be called without any parameters and return desired results.
'''
orientation = []
gyroscope = []
accelerometer = []
...
class MyoRaw(object):
    ...
    def getEMG(self):
            #We need this to return EMG Array
            emg = [random.randint(0,100),random.randint(0,100),random.randint(0,100),random.randint(0,100),random.randint(0,100),random.randint(0,100),random.randint(0,100),random.randint(0,100)]
            print(emg)
    def getRoll(self):
            #We need this to return Roll
            return None
    def getPitch(self):
            #We need this to return Pitch
            return None
    def getYaw(self):
            #We need this to return Yaw
            return None
...
def myo_Script(user):
	...
	if(pistalGrip == 'Pose.Rest'):
		print('Pose Rest Detected')

	try:
		while True:
				#THESE WILL BE DELETED WHEN YOU REPAIR THE LOWER CALLS TO THE SETTER METHODS
			...
			user.setRoll(random.randint(0,100))
			user.setPitch(random.randint(0,100))
			user.setYaw(random.randint(0,100))


			#if(m.shotFired()):
				#user.setShot()
			#user.setEMG(m.getEMG())
			#user.setRoll(m.getRoll())
			#user.setPitch(m.getPitch())
			#user.setYaw(m.getYaw())
	except:
		print("Error in Myo Script")
	finally:
		m.disconnect()
